...
String languageOption[] = { "english", "japanese", "spanish","italian"};
int numOfLanguageOption = sizeof(languageOption) / sizeof(languageOption[0]);
...
void optionSelect(String header) {
  ...
  if (buttonHandler() == "select" && menu[upDownCounter] != "") {
...
// --- PAGE CONTROLLER ---
String page_option[] = { "selectWifi", "typeWifiPassword", "mainMenu", "translate","languageSetting"};
...
void typeWifiPasswordPage() {
  ...
  if (String(getData("ssid")) == ssid && String(getData("ssid")) != "") {
    if (!connectWifi(String(getData("ssid")), String(getData("password")), 10000)) {
      addData("ssid", "");
      addData("password", "");
    }
    delay(3000);
  ...
  if (buttonHandler() == "select") {
    ...
    if (connectWifi(ssid, password, 10000)) {
      addData("ssid", ssid);
...
void mainMenu() {
  String menu[] = { String(page_option[0]), String(page_option[3]), String(page_option[4])};
  int menuSize = sizeof(menu) / sizeof(menu[0]);
  setMenu(menu, menuSize);
...
int findLanguageIdx(String lang){
  for (int i = 0; i < numOfLanguageOption; i++){
    if (languageOption[i] == lang){
      return i;
    }
  }
  return -1;
...
void languageSetting(){
  if (!trigger){
    trigger = true;
    upDownCounter = findLanguageIdx(sourceLanguage);
  }
  ...
  if (digitalRead(RECORD_BUTTON2) == HIGH){
    leftRightCounter = 1;
    upDownCounter = findLanguageIdx(outputLanguage);
  }

  if (buttonHandler() == "up" && upDownCounter > 0) upDownCounter -= 1;
  if (buttonHandler() == "down" && upDownCounter < numOfLanguageOption-1) upDownCounter += 1;

  if (leftRightCounter == 0) sourceLanguage = languageOption[upDownCounter];
  if (leftRightCounter == 1) outputLanguage = languageOption[upDownCounter];

  String src = (leftRightCounter == 0) ? sourceLanguage+'<' : sourceLanguage;
  String out = (leftRightCounter == 1) ? '>'+outputLanguage : outputLanguage;


  display.clearDisplay();
  updateScreen("Language Setting", 1, getXMid("Language Setting"),0);

  updateScreen("B1", 1, 0,getYMid()-10);
  updateScreen(src, 1, 0,getYMid());

  updateScreen("B2", 1, getXRight(outputLanguage),getYMid()-10);
  updateScreen(out, 1, getXRight(out),getYMid());
...
void changePage(int pageIdx) {
  ...
  if (page_option[pageIdx] == "selectWifi") {
    scanningWifiTimer = -1;   // FORCE rescan
  }
}
...
void setup() {
  ...
  scanningWifi();
  // auto connect wifi if saved ssid scanned
  for (int i = 0; i < availableWifiSize; i++){
    if (availableWifi[i] == String(getData("ssid")) && String(getData("ssid")) != ""){
      if (!connectWifi(String(getData("ssid")),String(getData("password")))){
        addData("ssid", "");
        addData("password", "");
        changePage(0);
      }
...
void loop() {
  if (current_page == page_option[0]) selectWifiPage();
  if (current_page == page_option[1]) typeWifiPasswordPage();
  if (current_page == page_option[2]) mainMenu();
  ...
  if (current_page == page_option[4]) languageSetting();
}